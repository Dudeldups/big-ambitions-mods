...
for index in range(3):
    ...
    assert np.max(abs(x)) < .8, 'Excessive transient or insufficient headroom'
    ...
    assert .15 < opening < .35 and body > .60, 'Attack is too soft or dominates the body'
    ...
    for pitch in (pitch_min, pitch_max):
        ...
        assert 1200 < centroid < 2800, 'Pop is too dull or too bright'
        assert low_mid > .60 and .15 < highs < .35, 'Pop lost its crackle or regained excessive clack'
