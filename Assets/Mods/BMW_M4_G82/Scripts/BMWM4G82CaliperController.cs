...
internal sealed class BMWM4G82CaliperController : MonoBehaviour
{
    ...
    public void Initialize(VehicleController controller, ModContext? modContext)
    {
        ...
        for (var index = 0; index < BindingNames.GetLength(0); index++)
        {
            ...
            ValidateSerializedCaliperGeometry(pivot);
        ...
        context?.Logger.Info(
            ...
            $"bindings={bindings.Count}, serializedRenderersPreserved=true, " +
            "followsSteeringAndSuspension=true, inheritsWheelSpin=false.");
    }
    ...
    private static void ValidateSerializedCaliperGeometry(Transform pivot)
    {
        var renderer = pivot.GetComponentInChildren<MeshRenderer>(true) ??
                       throw new InvalidOperationException(
                           $"Serialized caliper renderer below '{pivot.name}' is missing.");
        var filter = renderer.GetComponent<MeshFilter>();
        if (filter == null || filter.sharedMesh == null)
            throw new InvalidOperationException(
                $"Serialized caliper mesh below '{pivot.name}' is missing.");
    }
