...
internal sealed class BMWM4G82DriverController : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                lastFailure = reason;
                context?.Logger.Warn($"BMWM4G82 driver vehicle={vehicle.GetInstanceID()} " +
    ...
    private void CreateDriver()
    {
        ...
        if (steeringWheel == null)
            throw new InvalidOperationException("M4 steering-wheel seat reference is missing.");
        ...
        driverRoot = new GameObject("BMWM4G82_SeatedPlayer");
        ...
        poseGraph = PlayableGraph.Create("BMWM4G82 seated player");
    ...
    private void LogInfo(string message) =>
        context?.Logger.Info($"BMWM4G82 driver vehicle={vehicle?.GetInstanceID()}: {message}");
