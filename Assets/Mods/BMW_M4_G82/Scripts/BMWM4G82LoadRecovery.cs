...
internal static class BMWM4G82LoadRecovery
{
    ...
    private static readonly FieldInfo? IndoorPlacementCameraField = typeof(GameManager).GetField(
        "indoorPlacementCamera",
        BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
    private static readonly PropertyInfo? IndoorPlacementCameraProperty =
    ...
    internal static bool CompleteInterruptedLoad(ModContext? context)
    {
        ...
        foreach (var vehicle in save.privateDriverVehicleInstances)
            hasBmwM4 |= vehicle != null && string.Equals(vehicle.vehicleTypeName,
                BMWM4G82Mod.VehicleTypeName, StringComparison.Ordinal);
        if (!hasBmwM4)
            return false;

        attemptedManager = manager;
        var exitRegistered = HasNativeExitCallback(manager);
        var indoorPlacementCamera = GetIndoorPlacementCamera(manager);
        BMWM4G82Diagnostics.LoadRecoveryInfo(context, $"BMWM4G82: chauffeur save startup check: " +
        ...
        {
            context.Logger.Warn("BMWM4G82: startup recovery skipped: native initialization state differs from the supported interrupted-Awake path.");
            return false;
        }
        ...
        foreach (var vehicle in save.VehicleInstances)
        {
            ...
            {
                context.Logger.Warn($"BMWM4G82: startup recovery stopped: player vehicle type still unresolved: '{vehicle?.vehicleTypeName ?? "<null record>"}'.");
                return false;
            }
        }
        foreach (var vehicle in save.privateDriverVehicleInstances)
        {
            if (vehicle == null || vehicle.VehicleType == null)
            {
                context.Logger.Warn($"BMWM4G82: startup recovery stopped: chauffeur vehicle type still unresolved: '{vehicle?.vehicleTypeName ?? "<null record>"}'.");
                return false;
            }
        }
        ...
        try
        {
            ...
            context.Logger.Warn($"BMWM4G82: recovered interrupted city startup after vehicle registration; " +
        ...
        catch (Exception exception)
        {
            ...
            context.Logger.Warn($"BMWM4G82: interrupted-load recovery failed: {failure}");
            return false;
        }
    }
    ...
    internal static void ReportInputState(ModContext? context)
    {
        ...
        if (!recoveredManager.isActiveAndEnabled || !InputHelper.IsInitialized())
            context.Logger.Warn($"BMWM4G82: recovered city is not ready for input: " +
                $"managerActive={recoveredManager.isActiveAndEnabled}, inputInitialized={InputHelper.IsInitialized()}.");
        if (!BMWM4G82Diagnostics.DebugEnabled || !BMWM4G82Diagnostics.LoadRecoveryDebugEnabled)
            return;
        BMWM4G82Diagnostics.LoadRecoveryInfo(context, $"BMWM4G82: post-load input state: " +
            $"managerActive={recoveredManager.isActiveAndEnabled}, inputInitialized={InputHelper.IsInitialized()}, " +
            $"loading={UI.Load.LoadScene.isLoading}, " +
    ...
    private static Component? GetIndoorPlacementCamera(GameManager manager) =>
        (IndoorPlacementCameraField?.GetValue(manager) ??
         IndoorPlacementCameraProperty?.GetValue(manager)) as Component;
}
