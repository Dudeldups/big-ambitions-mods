...
internal static class BMWM4G82PrivateDriverSupport
{
    ...
    private const string AiPrefabCacheKey = "Prefabs/Vehicles/bmwm4g82.prefab";
    private const string AiCarTypeName = "BMWM4G82PrivateDriver";
    ...
    internal static void RemoveVehicle(string vehicleTypeName)
    {
        ...
        if (modifiedVehiclePool != null && customCarType != null)
        {
                     ...
                     ReferenceEquals(customCarType.vehiclePrefab, customAiPrefab))
            {
                ...
                // BMW prefab and its cached colors alive instead of invalidating
    ...
    private static GameObject? CreateAiPrefab(GameObject playerPrefab)
    {
        ...
        clone.name = "BMWM4G82PrivateDriver";
        ...
        var requiredVisuals = new[]
        {
            ...
            "BMWWheelRearLeft",
            "BMWWheelRearRight",
            "BMWFixedCaliperFrontLeft",
            "BMWFixedCaliperFrontRight",
            "BMWFixedCaliperRearLeft",
            "BMWFixedCaliperRearRight",
        };
        ...
        var materialOwner = clone.AddComponent<BMWM4G82RuntimeMaterialOwner>();
        materialOwner.Initialize();
        BMWM4G82Materials.FixSolidMaterials(clone);
        BMWM4G82Materials.ApplyCaliperFinish(clone);
        foreach (var renderer in templateRenderers)
            renderer.enabled = false;
        clone.AddComponent<BMWM4G82AmbientTrafficAppearance>();
        if (!FitAiBodyColliders(clone, playerPrefab))
        {
            UnityEngine.Object.Destroy(clone);
            return null;
        }
        var appearance = clone.AddComponent<BMWM4G82PrivateDriverAppearance>();
    ...
    private static bool FitAiBodyColliders(GameObject clone, GameObject playerPrefab)
    {
        ...
        {
            Debug.LogWarning("BMWM4G82 NPC body collider source is missing.");
            return false;
        }
        ...
        var holder = new GameObject("BMWM4G82 NpcBodyCollider");
        ...
        if (front <= rear)
        {
            Debug.LogWarning("BMWM4G82 NPC body colliders have invalid bounds.");
            UnityEngine.Object.Destroy(holder);
            return false;
        }

        FitAiNavigationObstacles(clone, left, right, rear, front);
        BMWM4G82Diagnostics.TrafficInfo($"BMWM4G82 NPC body boxes={holder.GetComponents<BoxCollider>().Length} " +
        ...
        GameObject clone, float bodyLeft, float bodyRight, float bodyRear, float bodyFront)
    {
        ...
        foreach (var obstacle in clone.GetComponentsInChildren<NavMeshObstacle>(true))
        {
            ...
            {
                Debug.LogWarning($"BMWM4G82 NPC obstacle could not be fitted name='{obstacle.name}'.");
                continue;
            }
            ...
            {
                Debug.LogWarning($"BMWM4G82 NPC obstacle has invalid fitted length name='{obstacle.name}'.");
                continue;
            }
            ...
            obstacle.size = size;
            BMWM4G82Diagnostics.TrafficInfo($"BMWM4G82 NPC obstacle name='{obstacle.name}' " +
...
internal sealed class BMWM4G82AmbientTrafficAppearance : MonoBehaviour
{
    ...
    {
        if (!BMWM4G82Diagnostics.TrafficEnabled || stateLogs >= 12)
            return;
        ...
        stateLogs++;
        BMWM4G82Diagnostics.TrafficInfo(
            $"BMWM4G82 NPC id={GetInstanceID()} moving={moving} " +
    ...
    private IEnumerator ApplyNativeTrafficColor()
    {
        ...
        if (GetComponent<PrivateDriverVehicle>() == null)
        {
            ...
            if (color != null)
            {
                var paint = GetComponent<BMWM4G82PaintController>();
                if (paint == null)
                    paint = gameObject.AddComponent<BMWM4G82PaintController>();
                paint.InitializeForPrivateDriver(color.name, color);
                applied = paint.HasAppliedColor;
            }
            if (colorLogs++ < 8)
                BMWM4G82Diagnostics.TrafficInfo($"BMWM4G82 ambient NPC id={GetInstanceID()} " +
                    $"color='{(color != null ? color.name : "<none>")}' applied={applied}.");
            if (color != null && !applied && colorLogs <= 8)
                Debug.LogWarning($"BMWM4G82 ambient NPC id={GetInstanceID()} failed to apply '{color.name}'.");
        }
        initializationCoroutine = null;
    }
}
...
internal sealed class BMWM4G82PrivateDriverAppearance : MonoBehaviour
{
    ...
    {
        { "BMWWheelFrontLeft", "FL" },
        { "BMWWheelFrontRight", "FR" },
        { "BMWWheelRearLeft", "BL" },
        { "BMWWheelRearRight", "BR" },
    };
    ...
    internal void BindWheelVisuals()
    {
        ...
        BindCaliper("BMWFixedCaliperFrontLeft", "BMWWheelFrontLeft");
        BindCaliper("BMWFixedCaliperFrontRight", "BMWWheelFrontRight");
        BindCaliper("BMWFixedCaliperRearLeft", "BMWWheelRearLeft");
        BindCaliper("BMWFixedCaliperRearRight", "BMWWheelRearRight");
    ...
    private bool RestoreSavedVehicleColor(bool applySpecializedPaint)
    {
        ...
        if (BMWM4G82PrivateDriverSupport.TryResolveDriverColor(
                colorName,
