...
internal static class BugattiChironLoadRecovery
{
    ...
    internal static bool CompleteInterruptedLoad(ModContext? context)
    {
        ...
        var hasBugatti = false;
        foreach (var vehicle in save.privateDriverVehicleInstances)
        {
            hasBugatti |= vehicle != null && string.Equals(
        ...
        BugattiChironDiagnostics.LoadRecoveryInfo(
            context,
            $"BugattiChiron: chauffeur save startup check: " +
            ...
            $"playerRecords={save.VehicleInstances?.Count ?? 0}, " +
            $"chauffeurRecords={save.privateDriverVehicleInstances.Count}.");
        if (exitRegistered)
            return false;
        ...
        {
            context.Logger.Warn(
                "BugattiChiron: startup recovery skipped: native initialization state differs from the supported interrupted-Awake path.");
            return false;
        }
        ...
        foreach (var vehicle in save.VehicleInstances)
        {
            ...
            {
                context.Logger.Warn(
                    $"BugattiChiron: startup recovery stopped: player vehicle type still unresolved: " +
                    $"'{vehicle?.vehicleTypeName ?? "<null record>"}'.");
                return false;
            }
        }
        foreach (var vehicle in save.privateDriverVehicleInstances)
        {
            ...
            {
                context.Logger.Warn(
                    $"BugattiChiron: startup recovery stopped: chauffeur vehicle type still unresolved: " +
                    $"'{vehicle?.vehicleTypeName ?? "<null record>"}'.");
                return false;
            }
        }
        ...
        try
        {
            ...
            {
                foreach (var group in UnityEngine.Object.FindObjectsByType<LODGroup>(
            ...
            context.Logger.Warn(
                $"BugattiChiron: recovered interrupted city startup after vehicle registration; " +
                $"managerWasDisabled={wasDisabled}, managerEnabled={manager.enabled}. " +
                "Native Start/Update can resume.");
            return true;
        }
        catch (Exception exception)
        {
            var failure = exception is TargetInvocationException invocation &&
                          invocation.InnerException != null
                ? invocation.InnerException
                : exception;
            context.Logger.Warn(
                $"BugattiChiron: interrupted-load recovery failed: {failure}");
            return false;
        }
    }
    ...
    internal static void ReportInputState(ModContext? context)
    {
        ...
        {
            context.Logger.Warn(
                $"BugattiChiron: recovered city is not ready for input: " +
                $"managerActive={recoveredManager.isActiveAndEnabled}, " +
                $"inputInitialized={InputHelper.IsInitialized()}.");
        }

        if (!BugattiChironDiagnostics.DebugEnabled ||
            !BugattiChironDiagnostics.LoadRecoveryDebugEnabled)
        {
            return;
        }
        ...
        BugattiChironDiagnostics.LoadRecoveryInfo(
            context,
            $"BugattiChiron: post-load input state: " +
            $"managerActive={recoveredManager.isActiveAndEnabled}, " +
            $"inputInitialized={InputHelper.IsInitialized()}, eventSystem={events != null}, " +
            $"enabled={events != null && events.isActiveAndEnabled}, " +
            $"module='{events?.currentInputModule?.GetType().Name ?? "<none>"}', " +
            $"centerUiHit='{top}', loading={UI.Load.LoadScene.isLoading}, " +
            $"nativeAwakeTail={HasNativeExitCallback(recoveredManager)}.");
    }
