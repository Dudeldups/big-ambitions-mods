...
internal sealed class BugattiChironPaintController : MonoBehaviour
{
    ...
    private void FindPaintSlots()
    {
        ...
        context?.Logger.Info(
            ...
            $"rimSlots={rimSlots}, interiorSlots={interiorSlots}; calipers/chrome/black excluded.");
        if (bodySlots == 0 || darkBodySlots == 0 || rimSlots == 0 || interiorSlots == 0)
            context?.Logger.Warn(
                $"BugattiChiron paint vehicle={vehicle?.GetInstanceID()}: " +
                $"paint mapping incomplete bodySlots={bodySlots}, darkBodySlots={darkBodySlots}, " +
                $"rimSlots={rimSlots}, interiorSlots={interiorSlots}.");
    }
    ...
    private void ApplyCurrentColor()
    {
        ...
        context?.Logger.Info(
            ...
            $"to {slots.Count} body/rim/interior slots.");
    }
