...
{
    internal sealed class CameraStoreMethodDetour
    {
        ...
        public CameraStoreMethodDetour(MethodInfo target, MethodInfo replacement)
        {
        ...
        public bool Restore(out string error)
        {
            ...
            try
            {
                ...
                {
                    error = "target entry changed after Camera Store installed its prefab resolver";
                    return false;
                }
        ...
        private static void WriteBytes(IntPtr address, byte[] bytes)
        {
            if (!VirtualProtect(address, new UIntPtr((uint)bytes.Length), PageExecuteReadWrite, out var previousProtection))
                throw new InvalidOperationException("VirtualProtect failed before writing the Camera Store method detour.");
