...
internal sealed class FerrariSF90SpiderCaliperController : MonoBehaviour
{
    private static readonly string[,] BindingNames =
    {
        { "FerrariSF90SpiderFixedCaliperFrontLeft", "FerrariSF90SpiderWheelFrontLeft" },
        { "FerrariSF90SpiderFixedCaliperFrontRight", "FerrariSF90SpiderWheelFrontRight" },
        { "FerrariSF90SpiderFixedCaliperRearLeft", "FerrariSF90SpiderWheelRearLeft" },
        { "FerrariSF90SpiderFixedCaliperRearRight", "FerrariSF90SpiderWheelRearRight" },
    };
    ...
    public void Initialize(VehicleController controller, ModContext? modContext)
    {
        ...
        FerrariSF90SpiderDiagnostics.Info(context,
            $"FerrariSF90Spider steering calipers ready vehicle={controller.GetInstanceID()}, " +
        ...
        foreach (var binding in bindings)
        {
            ...
            if (Mathf.Abs(steeringAngle) > 60f)
            {
                if (!warnedAboutInvalidSteering)
                {
                    ...
                    context?.Logger.Warn(
                        $"FerrariSF90Spider steering caliper rejected angle={steeringAngle:0.0}deg " +
