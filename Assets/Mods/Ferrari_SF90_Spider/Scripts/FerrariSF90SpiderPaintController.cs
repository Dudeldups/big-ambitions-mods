...
[AddComponentMenu("")]
public sealed class FerrariSF90SpiderPaintController : MonoBehaviour
{
    private const int PaintSettlementAttempts = 20;
    private const float PaintSettlementDelay = 0.10f;
    private const string BodyMaterialMarker = "2021Paint_Material";
    ...
    private void Awake()
    {
        ...
        FerrariSF90SpiderMaterials.FixSolidMaterials(controller.gameObject);
        Initialize(controller, null);

        var glassController = controller.GetComponent<FerrariSF90SpiderGlassController>();
        if (glassController == null)
            glassController = controller.gameObject.AddComponent<FerrariSF90SpiderGlassController>();
    ...
    {
        var applied = ApplyCurrentColor(true);
        if (!applied && settleWhenUnchanged)
            SchedulePaintSettlement();
        FerrariSF90SpiderDiagnostics.PaintInfo(
            context,
            $"FerrariSF90Spider paint refresh source='{source}' applied={applied}.");
        return applied;
    }
    ...
    private void FindPaintSlots()
    {
        ...
        FerrariSF90SpiderDiagnostics.Info(context,
            $"FerrariSF90Spider paint vehicle={vehicle?.GetInstanceID()}: " +
            $"mapped SF90 bodySlots={bodySlots}, caliperSlots={caliperSlots}, " +
            $"interiorAccentSlots={interiorAccentSlots}; " +
            "wheels, tires, carbon, black trim, windows, and lamp materials remain factory materials.");
        if (bodySlots == 0 || caliperSlots != 4)
            context?.Logger.Warn(
                $"FerrariSF90Spider paint mapping incomplete bodySlots={bodySlots}, " +
