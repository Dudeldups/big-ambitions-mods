...
/// <summary>
/// Starts native warehouse entry just before the SF90Spider's low front reaches the
...
internal sealed class FerrariSF90SpiderWarehouseEntryController : MonoBehaviour
{
    ...
    private static readonly Vector3 FrontProbePosition = new Vector3(0f, 0.47f, 2.42f);
    private static readonly Vector3 FrontProbeSize = new Vector3(0.42f, 0.62f, 0.60f);
    ...
    private void EnsureFrontProbe()
    {
        ...
        var probeObject = new GameObject("FerrariSF90Spider_WarehouseFrontProbe")
        {
    ...
    internal void SuppressEntrance(DriveInEntrance entrance, string reason)
    {
        ...
        FerrariSF90SpiderDiagnostics.WarehouseInfo(
            context,
            $"FerrariSF90Spider warehouse-entry: suppressing exact-door fallback for " +
    ...
    internal void ClearSuppressedEntrance(DriveInEntrance? entrance, string reason)
    {
        ...
        FerrariSF90SpiderDiagnostics.WarehouseInfo(
            context,
            $"FerrariSF90Spider warehouse-entry: restoring exact-door fallback for " +
        ...
        Vector3 relativeVelocity)
    {
        ...
        if (fallbackSuppressed)
        {
            if (Time.unscaledTime >= nextSuppressionLogTime)
            {
                ...
                FerrariSF90SpiderDiagnostics.WarehouseInfo(
                    context,
                    $"FerrariSF90Spider warehouse-entry: fallback suppressed for all doors during " +
        ...
        FerrariSF90SpiderDiagnostics.WarehouseInfo(
            context,
            $"FerrariSF90Spider warehouse-entry: exact entrance invokes native entry; " +
