...
internal sealed class KoenigseggJeskoAudioController : MonoBehaviour
{
    ...
    private bool TryConfigure()
    {
        ...
        crackleClip = KoenigseggJeskoCrackleWave.Create();
        var exhaustHost = new GameObject("KoenigseggJesko_ExhaustCrackle");
        ...
        var hornHost = new GameObject("KoenigseggJesko_Horn");
    ...
    {
        var clip = KoenigseggJeskoWave.Load(Path.Combine(context!.ModRootPath, "Config", "Audio", name + ".wav"));
        ownedClips.Add(clip);
        return clip;
    }
    ...
    private void UpdatePlayback()
    {
        ...
        if (!paused)
        {
            ...
            var normalized = KoenigseggJeskoAudioModel.Normalize(smoothRpm, engine.idleRPM, engine.revLimiterRPM);
            ...
            gain *= Mathf.Lerp(1.28f, 1f, normalized);
            loadBlend = KoenigseggJeskoAudioModel.LoadBlend(smoothThrottle);
            for (var i = 0; i < EngineNames.Length; i++)
            {
                layers[i].pitch = KoenigseggJeskoAudioModel.Pitch(normalized,i);
                layers[i + 3].pitch = layers[i].pitch;
                var bandGain = gain * KoenigseggJeskoAudioModel.Weight(normalized, i);
            ...
            crackleSource.volume = envelope * master * Mathf.Lerp(
                KoenigseggJeskoAudioModel.CrackleIdleVolume,
                KoenigseggJeskoAudioModel.CrackleLoadVolume,
                crackleLoad);
    ...
    private void UpdateHorn(bool pressed, float master)
    {
        ...
        var lowTarget = pressed ? master * KoenigseggJeskoAudioModel.HornLowVolume : 0f;
        var highTarget = pressed ? master * KoenigseggJeskoAudioModel.HornHighVolume : 0f;
    ...
    private void Warn(string message) => context?.Logger.Warn($"KoenigseggJesko audio vehicle={vehicle?.GetInstanceID()}: {message}");
