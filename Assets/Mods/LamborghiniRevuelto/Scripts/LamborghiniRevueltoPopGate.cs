...
internal enum LamborghiniRevueltoPopEvent { None, ThrottleLift, Downshift, Upshift }
...
internal sealed class LamborghiniRevueltoPopGate
{
    ...
    private LamborghiniRevueltoPopEvent burstReason;
    ...
    internal LamborghiniRevueltoPopGate(int? seed = null)
    {
    ...
    private void TryBurst(LamborghiniRevueltoPopEvent reason, double time, float chance, float strength, int maxPops, float eventRpm, int fromGear)
    {
        DecisionId++;
        Probability = chance;
        EventRpm = eventRpm;
        ...
        cooldownUntil = time + (reason == LamborghiniRevueltoPopEvent.Downshift ? 1.0d : .8d) +
            random.NextDouble()*(reason == LamborghiniRevueltoPopEvent.Downshift ? .35d : .25d);
    }

    internal LamborghiniRevueltoPopEvent Sample(bool active, double time, float rpm, float throttle, int gear, float speedKph = 0f)
    {
        if (!active || gear <= 0)
        {
            Reset();
            return LamborghiniRevueltoPopEvent.None;
        }
        ...
        if (!double.IsPositiveInfinity(downshiftUntil))
        {
            ...
            if (time >= downshiftUntil)
            {
                ...
                TryBurst(LamborghiniRevueltoPopEvent.Downshift, time, chance, strength, 2, downshiftPeak, downshiftFromGear);
                downshiftUntil = double.PositiveInfinity;
            }
        }
        ...
        if (continuous && armed && throttle <= .2f && previousThrottle > .2f)
        {
            ...
            TryBurst(LamborghiniRevueltoPopEvent.ThrottleLift, time, chance, strength, 3, rpm, previousGear);
        }
        else if (continuous && armed && (previousGear == 1 || previousGear == 2) && gear == previousGear+1)
        {
            ...
            RpmJump = 0f;
            TryBurst(LamborghiniRevueltoPopEvent.Upshift, time, chance, .75f*strength, 2, previousRpm, previousGear);
        }
        ...
        previousGear = gear;
        valid = true;
        if (rpm < 1400f || (throttle > .4f && burstReason == LamborghiniRevueltoPopEvent.ThrottleLift)) remaining = 0;
        if (remaining <= 0 || time < nextPop) return LamborghiniRevueltoPopEvent.None;
        remaining--;
