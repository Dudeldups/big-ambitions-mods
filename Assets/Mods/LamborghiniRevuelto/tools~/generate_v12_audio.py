"""Generate deterministic, seamless synthetic Lamborghini V12 engine layers.
...
LAYERS = (("EngineLow", 100.0), ("EngineMid", 350.0), ("EngineHigh", 750.0))
...
def layer(reference_hz: float, loaded: bool, seed: int) -> list[float]:
    ...
    for index in range(COUNT):
        ...
        # A 50 Hz periodic intake pulse adds low-end body without a borrowed recording.
        value += (0.18 if loaded else 0.08) * math.sin(math.tau * 50.0 * time + phases[0])
        if loaded:
...
def write_wav(path: Path, samples: list[float]) -> dict[str, float | int | str]:
    ...
    return {
        ...
        "peak": peak * scale,
        "method": "deterministic additive V12 synthesis",
    }
...
def main() -> None:
    ...
    report: dict[str, object] = {
        ...
        "firing_frequency": "six combustion events per crankshaft revolution",
