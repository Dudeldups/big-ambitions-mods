...
internal sealed class Porsche911GT3RSAudioController : MonoBehaviour
{
    ...
    {
        var clip = Porsche911GT3RSWave.Load(Path.Combine(context!.ModRootPath, "Config", "Audio", name + ".wav"));
        ownedClips.Add(clip);
        return clip;
    }
    ...
    private void UpdatePlayback()
    {
        ...
        if (!paused)
        {
            ...
            var normalized = Porsche911GT3RSAudioModel.Normalize(smoothRpm, engine.idleRPM, engine.revLimiterRPM);
            ...
            for (var i = 0; i < EngineNames.Length; i++)
            {
                layers[i].pitch = Porsche911GT3RSAudioModel.Pitch(normalized,i);
                layers[i + 3].pitch = layers[i].pitch;
                var bandGain = gain * Porsche911GT3RSAudioModel.Weight(normalized, i);
    ...
    private void UpdateHorn(bool pressed, float master)
    {
        ...
        var lowTarget = pressed ? master * Porsche911GT3RSAudioModel.HornLowVolume : 0f;
        var highTarget = pressed ? master * Porsche911GT3RSAudioModel.HornHighVolume : 0f;
    ...
    private void Warn(string message) => context?.Logger.Warn($"Porsche911GT3RS audio vehicle={vehicle?.GetInstanceID()}: {message}");
