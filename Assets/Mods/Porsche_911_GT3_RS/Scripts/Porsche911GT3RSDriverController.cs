...
internal sealed class Porsche911GT3RSDriverController : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                lastFailure = reason;
                context?.Logger.Warn($"Porsche911GT3RS driver vehicle={vehicle.GetInstanceID()} " +
    ...
    private void CreateDriver()
    {
        ...
        foreach (var child in vehicle!.GetComponentsInChildren<Transform>(true))
            if (child.name.IndexOf(SteeringWheelMarker, StringComparison.OrdinalIgnoreCase) >= 0)
                steeringWheel = child;
        if (steeringWheel == null)
            throw new InvalidOperationException("GT3 RS steering-wheel seat reference is missing.");
        ...
        driverRoot = new GameObject("Porsche911GT3RS_SeatedPlayer");
        ...
        poseGraph = PlayableGraph.Create("Porsche911GT3RS seated player");
