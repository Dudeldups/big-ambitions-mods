"""Generate deterministic, seamless synthetic Porsche flat-six engine layers.
...
LAYERS = (("EngineLow", 70.0), ("EngineMid", 220.0), ("EngineHigh", 420.0))
...
def layer(reference_hz: float, loaded: bool, seed: int) -> list[float]:
    ...
    for index in range(COUNT):
        ...
        # A 45 Hz periodic pulse evokes the 900 rpm flat-six idle without samples.
        value += (0.20 if loaded else 0.10) * math.sin(math.tau * 45.0 * time + phases[0])
        if loaded:
...
def write_wav(path: Path, samples: list[float]) -> dict[str, float | int | str]:
    ...
    return {
        ...
        "peak": peak * scale,
        "method": "deterministic additive naturally aspirated flat-six synthesis",
    }
